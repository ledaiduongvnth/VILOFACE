...
class AdaptiveWingLoss(nn.Module):
    def __init__(self, w=14, e=1, theta=0.5, alpha=2.1):
    ...
    def forward(self, x, t, sigma=1):
        ...
        A = self.w * (1 / (1 + torch.pow(self.theta / self.e, self.alpha - t))) * (self.alpha - t) * torch.pow(self.theta / self.e, self.alpha - t - 1) * (1/ self.e)
        C = (self.theta * A - self.w * torch.log(1 + torch.pow(self.theta / self.e, self.alpha - t)))
        weight = torch.ones_like(x)
        weight[torch.where(t==0)] = 0
        diff = (x - t)
        abs_diff = diff.abs()
        flag = (abs_diff.data < self.theta).float()
        y = flag * self.w * torch.log(1 + torch.pow(abs_diff / self.e, self.alpha - t)) + (1 - flag) * (A * abs_diff - C)
        return y.sum()
...
class KPTLoss(nn.Module):
    ...
    def __init__(self, alpha=1.0):
        ...
        # self.loss_fcn = AdaptiveWingLoss()#nn.SmoothL1Loss(reduction='sum')
        self.alpha = alpha
